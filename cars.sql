...
2. select count(*) as quantity from cars;
...
6. select count(year_of_issue) from cars where brand = 'Chevrolet'
...
13.select max(price) as most_expensive, min(price) as cheaper from cars;
...
17.select * from cars where year_of_issue not between 1995 and 2005;
   select * from cars where year_of_isue not between 1995 and 2005 group by desc;
